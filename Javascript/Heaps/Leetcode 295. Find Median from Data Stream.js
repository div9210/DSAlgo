...
class MedianFinder {
  constructor() {
    this.maxH = new PriorityQueueI((a, b) => b - a);
    this.minH = new PriorityQueueI((a, b) => a - b);
    this.median = 0;
  }
  ...
  addNum(num) {
    ...
    // Correct the heaps (if needed)
    while (this.minH.size() > 0 && this.maxH.peek() > this.minH.peek()) {
      let pickFromMax = this.maxH.delete();
      this.minH.insert(pickFromMax);
    }

    // Check the size difference
    let maxSize = this.maxH.size();
    let minSize = this.minH.size();
    let diff = Math.abs(maxSize - minSize);


    if (diff == 1) {
      // Pick median from who is bigger
      if (minSize > maxSize)
        this.median = this.minH.peek();
      else
        this.median = this.maxH.peek();
    } else if (diff == 2) {
      if (maxSize > minSize) {
        // Pick from maxH and put it in minH
        let pickFromMaxH = this.maxH.delete();
        this.minH.insert(pickFromMaxH);
      } else {
        // Pick from minH and put it in maxH
        let pickFromMinH = this.minH.delete();
        this.maxH.insert(pickFromMinH);
      }

      let max = this.maxH.peek();
      let min = this.minH.peek();
