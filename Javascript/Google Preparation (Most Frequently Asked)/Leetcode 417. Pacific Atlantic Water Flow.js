...
var pacificAtlantic = function (heights) {
    // Reverse Engineering
    // Rather than visiting each cell and finding out if water from it
    ...
    let pacificVisited = Array(heights.length).fill().map(() => Array(heights[0].length).fill(false));
    ...
    let atlanticVisited = Array(heights.length).fill().map(() => Array(heights[0].length).fill(false));
    for (let col = 0; col < heights[0].length; col++) {
    ...
    function dfs(srcx, srcy, visited) {
        ...
        for (let [dx, dy] of directions) {
            ...
            if (inBound(newx, newy, heights) && heights[newx][newy] >= heights[srcx][srcy] && !visited[newx][newy]) {
